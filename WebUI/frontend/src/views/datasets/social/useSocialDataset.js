...
export function useSocialDataset(datasetId) {
    ...
    const mergeSocialResults = (results) => {
    ...
    const fetchSocialResults = async (dataIds) => {
        const ids = dataIds && dataIds.length > 0 ? dataIds : dataList.value.map((item) => item.id);
        if (ids.length === 0)
            return;
        const res = await axios.post("/datasets/social/results", {
