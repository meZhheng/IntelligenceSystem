...
export type AnalysisType = "emotion" | "stance";
...
export function useSocialAnalysis(
  datasetId: DataId,
  dataList: Ref<SocialDataItem[]>,
  selectedItems: Ref<DataId[]>,
  currentConfig: Ref<AnalysisConfig>,
  refreshSocialResults: (dataIds?: DataId[]) => Promise<void>
) {
  const analysisType = ref<AnalysisType>("emotion");
  const analysisResults = ref<any[]>([]);
  const analysisSummary = ref<AnalysisSummary>({
    valid: 0,
    invalid: 0,
  ...
  const selectedData = () =>
    dataList.value.filter((item) => selectedItems.value.includes(item.id) || selectedItems.value.includes(String(item.id)));
  ...
  const analyzeStance = async (targets: string[]) => {
    if (selectedItems.value.length === 0) {
      ElMessage.warning("请先选择要分析的数据");
      return;
    }
