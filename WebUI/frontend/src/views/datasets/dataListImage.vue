...
<div class='data-list-image'>
    ...
    >
    <!-- 图片展示区 -->
    <div v-if='data.images && data.images.length' class='item-images'>
        <el-image
            :src='"/static/images/" + data.dataset_id + "/" + data.images[currentIndexes[data.id]].path'
            fit='cover'
            lazy
            class='item-image'
        />
        <button
            ...
            @click.stop='prevImage(data.id, data.images.length)'
        >
        ...
        <button
            ...
            @click.stop='nextImage(data.id, data.images.length)'
        >
...
const props = withDefaults(defineProps<{
    ...
    group_id: number
    task_type: string
    default?: boolean    // 新增可选 prop
}>(), {
    default: false       // 设置默认值
});

const select_data = (dataset_id: number, data_id: number) => {
    if (!props.default) {
            router.push({
            name: 'InferenceIndex',
            query: {
...
function prevImage(dataId: number, len: number) {
    const idx = currentIndexes.value[dataId]!;
    currentIndexes.value[dataId] = (idx - 1 + len) % len;
}

function nextImage(dataId: number, len: number) {
    const idx = currentIndexes.value[dataId]!;
