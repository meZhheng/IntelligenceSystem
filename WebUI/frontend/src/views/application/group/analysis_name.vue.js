...
export default (await import('vue')).defineComponent({
    ...
    methods: {
        async fetchGroupInfo(groupId) {
            const link = `/application/groups/13`;
        ...
        async checkGroupStatus() {
            const link = `/application/groups/${this.group.id}/checkGroupStatus`;
            const link2 = `/application/groups/name/checkGroupStatus`;
            // 示例：调用接口检查任务状态
            if (this.isChecked) {
        ...
        async fetchAnalysisResult() {
            ...
            const link2 = `/application/groups/name/analyze`;
        ...
        async fetchGroupStatus() {
            ...
            const link2 = `/application/groups/name/status`;
            axios.get(link2).then((response) => {
                ...
                this.datasetOptions = all_available_datasets
                    .filter(ds => ds.type_name === 'ILLEGAL_ACCOUNT_DETECTION')
                    .map(ds => ({
                    label: ds.label,
                    value: ds.value
                }));
...
const __VLS_74 = {}.AutoScrollWindow;
...
const __VLS_80 = {}.AutoScrollWindow;
/** @type { [typeof __VLS_components.AutoScrollWindow, ] } */ ;
// @ts-ignore
...
['item', 'item_header', 'item_header_left', 'item_header_title', 'total_info', 'info_item', 'main_items', 'sub_items', 'info_item', 'main_items', 'sub_items', 'info_item', 'main_items', 'sub_items', 'progress_bar', 'item_header_right', 'right_item', 'circle_description', 'main_items', 'right_item', 'circle_description', 'main_items', 'chart_list', 'chart_list_row',];
var __VLS_special;
let __VLS_self;
