...
export default (await import('vue')).defineComponent({
    ...
    data() {
        return {
            ...
            // '女兵喜欢训新兵，但是自己喜欢偷懒',
            // '男人呢？都轮到女人上战场了吗？',
            // '男兵女兵射击成绩相同，谁知连长看了靶纸后，当场判定男兵胜, 男兵女兵射击成绩相同, 当场判定男兵胜, 男兵女兵射击成绩相同',
            // '女兵跟男兵不一样，男兵还好，女兵的话，没关系想进去不容易的。',
            // '男兵多女兵少的原因很简单，那就是男兵更能打',
            // '毕竟 国之花瓶',
            // '不用去争论的，男女性别差异。我支持男女平等，可是战争让女人走开。',
            // '女兵多的地方是非多说的太对了',
            // '某飞行表演队有女飞，我认为仅具政治宣传意义，花瓶'
            // ],
            rows_neg: [],
            total_data: 0,
            statusTimer: null,
    ...
    computed: {
        ...
        passAnalysisStatus() {
            console.log("分析任务状态");
            console.log(this.analysisStatus);
    ...
    methods: {
        async fetchGroupInfo(groupId) {
            const link = `/application/groups/10`;
        ...
        async fetchDatasetStats() {
            ...
            try {
                ...
                responses.forEach((res, idx) => {
                    ...
                    if (target === 'distribution_station') {
                        this.datasetStats[target] = [
                            { name: "新浪微博", value: 566 },
                            { name: "小红书", value: 218 },
                            { name: "百度贴吧", value: 216 }
                        ];
        ...
        async fetchAnalysisResult() {
            ...
            const link2 = `/application/groups/emotion/analyze`;
            ...
            axios.post(link2, {
                ...
                // // 转换数据并传递给图表组件
                // this.$refs.chartComponent.chartData = this.adaptChartData(response.data.response);
            }).catch((error) => {
                console.error(error);
            });
        },
        async fetchRollingText() {
            const link = `/application/groups/emotion/RollingText`;
            const loadingTextStats = ElLoading.service({
                target: '.item_header_right',
                text: '加载中...',
            });
            axios.get(link, {
                ...
                // console.log('赋值前:', this.rows);
                this.rows_pos = response.data.text_pos;
                this.rows_neg = response.data.text_neg;
                // console.log('赋值后:', this.rows);
            }).catch((error) => {
                console.error(error);
            }).finally(() => {
                loadingTextStats.close();
            });
        },
        async fetchGroupStatus() {
            const link = `/application/groups/${this.group.id}/status`;
            const link2 = `/application/groups/emotion/status`;
...
const __VLS_112 = {}.AutoScrollWindow;
...
const __VLS_118 = {}.AutoScrollWindow;
/** @type { [typeof __VLS_components.AutoScrollWindow, ] } */ ;
// @ts-ignore
