import LineChart from './trendLine_stance.vue';
...
export default (await import('vue')).defineComponent({
    ...
    methods: {
        async fetchGroupInfo(groupId) {
            const link = `/application/groups/12`;
        ...
        async fetchAnalysisResult() {
            ...
            const link2 = `/application/groups/stance/analyze`;
        ...
        async fetchGroupStatus() {
            ...
            const link2 = `/application/groups/stance/status`;
...
for (const [item] of __VLS_getVForSourceType((__VLS_ctx.modelOptions))) {
    ...
    const __VLS_25 = __VLS_asFunctionalComponent(__VLS_24, new __VLS_24({
        key: ((item.value)),
        label: ((__VLS_ctx.name_trans[item.label])),
        value: ((item.value)),
