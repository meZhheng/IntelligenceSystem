            ...
            <div class="time_selector">
                ...
                >
                    <div class="select-actions">
                        <el-button type="text" @click.stop="selectAll">
                            <i class="el-icon-circle-check"></i> 全选
                        </el-button>
                        <el-button 
                            v-if="filterText && computedFilteredTargets.length > 0"
                            type="text" 
                            @click.stop="selectAllMatched"
                        >
                            <i class="el-icon-finished"></i> 全选匹配项 ({{ computedFilteredTargets.length }})
                        </el-button>
                    </div>
                    <el-option 
                        v-for="target in computedOptions"
                        :key="target.value"
                        :label="target.label"
                        :value="target.value"
                    />
...
export default {
    ...
    computed: {
        ...
        computedFilteredTargets() {
            if (!this.filterText) return this.availableTargets;
            return this.availableTargets.filter(target => 
                target.includes(this.filterText)
            );
        },
    ...
    methods: {
        ...
        // 目标筛选
        filterByTarget(data) {
            if (this.selectedTargets.length === 0) return data;
