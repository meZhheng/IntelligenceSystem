...
const offensiveConfidence = computed(() => Math.round(props.result.confidences?.[0]?.[0] * 100 || 0));
const safeConfidence = computed(() => Math.round(props.result.confidences?.[0]?.[1] * 100 || 0));
// 样式计算
const resultClass = computed(() => ({
    'safe-result': isSafe.value,
    'offensive-result': !isSafe.value
}));
const resultTitle = computed(() => isSafe.value ? '安全文本' : '检测到冒犯性内容');
