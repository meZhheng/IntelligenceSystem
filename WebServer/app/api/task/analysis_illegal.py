...
@bp.route('/application/groups/illegal/analyze', methods=['POST'])
@token_auth.login_required
def get_analyze_results_illegal(group_id = 11):
    """
    分析应用组违法违规敏感信息检测结果
    ---
    swagger: "2.0"
    tags:
      - Application Group - Illegal Sensitive Information Detection
    parameters:
      - name: group_id
        in: path
        type: integer
    ...
    assert group_id == 11
    current_app.logger.info("===== 已进入group_id==11 违法违规敏感信息检测 =====")
...
@bp.route('/application/groups/illegal/status', methods=['GET'])
@token_auth.login_required
def get_group_status_illegal(group_id = 11):
    """
    获取应用组违法违规敏感信息检测状态
    ---
    swagger: "2.0"
