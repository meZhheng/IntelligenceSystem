...
def compute_bleu(Xbatch, pred_batch, idx2word, eos_token_idx):
    # 将张量转换为文本列表
    Xtext = [[utils.tensor2text(X, idx2word, eos_token_idx)[1:-1]]  # 移除SOS/EOS标记
             for X in Xbatch.cpu().detach()]
    pred_text = [utils.tensor2text(pred, idx2word, eos_token_idx)[1:-1]
                 for pred in pred_batch.cpu().detach()]
    
    # 调整数据结构适配NLTK要求[1,6](@ref)
    references = [[sent[0].split()] for sent in Xtext]  # 转换为嵌套列表结构
    candidates = [sent.split() for sent in pred_text]
    
    # 计算BLEU-4分数（默认权重）
    bleu = corpus_bleu(references, candidates) * 100  # 转换为百分比形式
