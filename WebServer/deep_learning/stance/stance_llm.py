...
class Stance_LLM(LLMBaseModel):
    ...
    def stance_judgement(self, text: str, target :str) -> Result:
        ...
        try:
            ...
            # print(response)
            data = json.loads(response.choices[0].message.content)
            return Result(text=data['result']['text'],target=data['result']['target'],reason=data['result']['reason'],stance=data['result']['stance'])
        except Exception as e:
            print(f"主体识别失败: {str(e)}")
            return Result(text='',target='',reason='',stance='')
