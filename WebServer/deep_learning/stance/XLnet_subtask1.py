...
# MODEL_NAME = "chinese-roberta-wwm"
...
class XLNetStanceClassifier1(torch.nn.Module, BaseModel):
    def __init__(self, model_path="/webfile/checkpoints/stance/covid_epidemic_subtask1.pth", device='cuda', max_len=128):
        super().__init__()
    ...
    def get_total_steps(self):
        test_df = pd.read_csv("/webfile/test_data/covid_epidemic/test.csv")
    ...
    def test_model(self):
        ...
        # 初始化组件
        tokenizer = AutoTokenizer.from_pretrained(MODEL_NAME)
        dataloader = DataLoader(StanceDataset(test_df, tokenizer), batch_size=BATCH_SIZE, num_workers=0)
        self.eval()
        ...
        with torch.no_grad():
            for batch in dataloader:
                ...
                try:
                    ...
                    # 局部 catch，抛出带上下文的异常
                    raise RuntimeError(f"第 {step} 批次处理失败: {e}, 数据: {batch}") from e
...
if __name__ == "__main__":
    # 加载数据
    train_df = pd.read_csv("covid_epidemic/train.csv")
    val_df = pd.read_csv("covid_epidemic/validation.csv")
    test_df = pd.read_csv("covid_epidemic/test.csv")
    ...
    # 训练流程
    model = XLNetStanceClassifier1("covid_epidemic_subtask1.pth").to(device)
    optimizer = AdamW(model.parameters(), lr=LR)
    
    model.train(model, dataloaders, optimizer, label_encoder)
    model.test(model, test_loader, label_encoder)
    
    # # 保存模型
    # torch.save(model.state_dict(), "covid_epidemic_subtask1.pth")
    
    # # 预测示例
    # predictor = StancePredictor("covid_epidemic_subtask1.pth")
