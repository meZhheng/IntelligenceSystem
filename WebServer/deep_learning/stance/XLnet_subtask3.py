...
# # 模型定义
# class XLNetStanceClassifier(torch.nn.Module):
#     def __init__(self):
#         super().__init__()
#         self.xlnet = AutoModel.from_pretrained(MODEL_NAME)
#         self.dropout = torch.nn.Dropout(0.1)
#         self.classifier = torch.nn.Linear(self.xlnet.config.hidden_size, 3)
...
#         self.model = XLNetStanceClassifier().to(device)
#         self.model.load_state_dict(torch.load(model_path))
#         self.model.eval()
    
#     def predict(self, text, target):
...
#         )
#         prediction = label_encoder.inverse_transform([probs.argmax().item()])[0]
...
#         return {
#             "prediction": label_encoder.inverse_transform([probs.argmax().item()])[0],
#             "probabilities": dict(zip(label_encoder.classes_, probs.cpu().numpy()[0])),
...
#     清洗文本的完整处理流程：
#     1. 替换换行符、回车符、制表符为空格
#     2. 移除其他控制字符（ASCII 0-31和127-159）
#     3. 去除首尾空格
#     4. 合并多个连续空格为单个
#     """
#     if not isinstance(text, str):  # 处理非字符串类型
#         return ""
...
class XLNetStanceClassifier3(torch.nn.Module, BaseModel):
    def __init__(self, model_path="/webfile/checkpoints/stance/covid_epidemic_subtask3.pth", device='cuda', max_len=128):
        super().__init__()
    ...
    def get_total_steps(self):
        test_df = pd.read_csv("/webfile/test_data/zero_shot/raw_test_all_onecol.csv")
...
if __name__ == "__main__":
    # 加载数据
    train_df = pd.read_csv("./zero_shot/raw_train_all_onecol.csv")
    val_df = pd.read_csv("./zero_shot/raw_val_all_onecol.csv")
    test_df = pd.read_csv("./zero_shot/raw_test_all_onecol.csv")
    ...
    # 训练流程
    model = XLNetStanceClassifier3().to(device)
    ...
    # # 保存模型
    # torch.save(model.state_dict(), "covid_epidemic_subtask3.pth")
    
    # # 预测示例
    # predictor = StancePredictor("covid_epidemic_subtask3.pth")
