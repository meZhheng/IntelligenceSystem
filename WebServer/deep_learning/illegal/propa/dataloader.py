...
class PropDataset(Dataset):
    ...
    def collate_fn(self, batch):
        ...
        for item in batch:
            ...
            # <------------- edge index FD ------------->
            edge_index = [[], []]
            edge_index[0] = [int(index) + counter -1 for index in thread['edge_index'][0]]
            edge_index[1] = [int(index) + counter -1 for index in thread['edge_index'][1]]
            ...
            # <------------- ranking ------------->
            ranking_tmp = torch.arange(len(rel_pos), dtype=torch.long)[:self.top_k]
        ...
        return {
            'root_ids': rootFeature['input_ids'],
            'root_mask': rootFeature['attention_mask'],
            'edge_index_FD': torch.LongTensor(edge_index_FD),
            'edge_index_BD': torch.LongTensor(edge_index_BD),
            'abs_time': torch.LongTensor(absTimeList),
            'rel_pos': torch.LongTensor(relPosList),
            'ranking_indices': ranking_tmp,
            'post_feature': postFeatureList,
            'rootIndexs': torch.LongTensor(rootIndexs),
            'labels': torch.LongTensor(labelList)
        }
