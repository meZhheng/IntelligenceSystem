...
class Trainer(transformers.Trainer):
    ...
    def train(self):
        ...
        for epoch in range(int(self.config.num_train_epochs)):
            ...
            self.acc = self.eval(model, epoch)
    ...
    def eval(self, model, epoch):
        ...
        with torch.no_grad():
            ...
            # 计算各项指标
            f1_score_pos, f1_score_neg, macro_f1_score, acc = \
                self.calculate_metrics(prediction_lst, label_lst)
            eval_loss /= len(self.test_dataset.batches)

            # 直接 print 输出
            print(f"[Eval][Epoch {epoch}] Loss: {eval_loss:.4f}")
            print(f"[Eval][Epoch {epoch}] F1 Positive: {f1_score_pos:.4f}")
            print(f"[Eval][Epoch {epoch}] F1 Negative: {f1_score_neg:.4f}")
            print(f"[Eval][Epoch {epoch}] F1 Macro: {macro_f1_score:.4f}")
    ...
    def save_model(self, output_dir):
        ...
        torch.save(self.model, os.path.join(output_dir, f"Acc {self.acc:4f}.bin"))
