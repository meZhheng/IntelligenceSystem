...
class Roberta_Prototypical_BiGCN:
    ...
    def responseRanking(self, feature, rootIndex, ranking_indices):
        ...
        for i, root_idx in enumerate(rootIndex):
            ...
            rank_ids = ranking_indices
            batch = torch.index_select(batch, dim=0, index=rank_ids)
        ...
        for item in tmp:
            if len(item) == self.n_tokens:
                ...
                propagation_emb.append(torch.cat((item, torch.zeros((self.n_tokens - len(item), item.shape[1]))), dim=0))
        ...
        return propagation_emb, propagation_mask
    ...
    def cat_attention_mask(self, template_mask, propagation_mask, attention_mask):
        ...
        if template_mask is None:
            ...
            learnable_mask = torch.ones((attention_mask.size(0), self.n_tokens))
