...
CONFIG = {
    ...
    "output_file_path": "./emotion_analysis_results.csv",
...
def predict_emotion(text, tokenizer, model):
    ...
    system_prompt = "你是情感分类工具，仅输出该文本的情感，三选一，积极/消极/中立"
    messages = [
        {"role": "system", "content": system_prompt},
        {"role": "user", "content": f"文本：{text}，输出情感"}
    ]
    ...
    try:
        ...
        emotion = next(s for s in emotion_categories if s in cleaned_content)
    except:
        emotion = "中立"
...
def batch_emotion_analysis():
    ...
    df = df[["text", "emotion", "raw_output", "model"]]
